...
| M6 — UE 5.7 + 5.8 compatibility | ⏳ planned |

**Current branch:** `main` == `M4-UX` (both `e462c41`; M4 4.1+4.2 merged & pushed)  
**M4 scope (done):** 4.1 Plugin Settings + 4.2 Export to file — merged to `main` (`e462c41`). Dropped: 4.5 README URLs (Patreon/Boosty plans paused). Deferred: 4.3 partial-graph filter, 4.4 progress bar. See `.grok/Private/ROADMAP.md` §M4.  
**M6 — UE 5.8 compat (planned, full detail in ROADMAP §M6):** approach **A+C** = single source tree + version guards via `UE_VERSION_OLDER_THAN`, centralized in a new `BPR_Compat.h`; CI compile-matrix 5.7+5.8 is the reliability gate (shared with M5.2). Separate-branches-per-version rejected (drift risk). Free "forward fixes" (e.g. `USizeBox::bOverride_*` → setters, which exist in 5.7) can start now on 5.7 — task 6.0. Dev workflow: one plugin git repo; a separate lightweight host project per engine version with the plugin **junction/symlinked** in (not copied); `RunUAT BuildPlugin` compiles the plugin standalone with no host project needed.  
